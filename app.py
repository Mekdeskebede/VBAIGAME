...
class RealtimeConversation:
    def __init__(self):
        ...
        self.message_duration = 5  # Message displays for 5 seconds
        # Add SSL context
        self.ssl_context = ssl.create_default_context()
        # For development/testing only - in production, use proper certificate verification
        ...
        self.current_transcript = ""  # Add this to store accumulated transcript
    ...
    async def connect_websocket(self):
        ...
        try:
            ...
            self.ws = await websockets.connect(
                ...
                ssl=self.ssl_context  # Add SSL context to connection
            )
        ...
        try:
            while True:
                ...
                if data.get("type") == "response.text.delta":
                    ...
                    # Skip VQ image tokens
                    if not delta.startswith("<|vq_image_") and not delta.startswith("<|image_border_"):
                        self.current_transcript += delta
                        
                elif data.get("type") == "response.audio_transcript.delta":
                    delta = data.get("delta", "")
                    self.current_transcript += delta
                    # Convert accumulated text to speech when we get punctuation
                    if any(punct in delta for punct in ".!?,:"):
                        if self.current_transcript.strip():
                            self.speak_text(self.current_transcript.strip())
                ...
                # Check if this is the end of the response
                    ...
                    # Speak any remaining transcript
                    if self.current_transcript.strip():
                        self.speak_text(self.current_transcript.strip())
    ...
    async def send_audio(self, base64_audio):
        ...
        try:
            ...
            # Send commit message after audio
            await self.ws.send(json.dumps({
    ...
    def render_message(self, surface):
        """Render the current message on the given surface."""
        ...
        # Render message with fade out effect
        alpha = 255 * (1 - (elapsed_time / self.message_duration))
        text_surface = font.render(self.current_message, True, (255, 255, 255))
...
class Game3D:
    def __init__(self):
        ...
        self.realtime_conv = RealtimeConversation()
        self.voice_mode = False

    async def handle_voice_chat(self):
        ...
        try:
            ...
            if not await self.realtime_conv.ensure_connection():
                print("Failed to establish connection to OpenAI API")
                self.voice_mode = False
                return
            
            # Record and send audio
            base64_audio = await self.realtime_conv.record_audio()
            await self.realtime_conv.send_audio(base64_audio)
            
            # Get and display response
            response = await self.realtime_conv.receive_response()
            if response:
                self.realtime_conv.current_message = response
                self.realtime_conv.message_start_time = time.time()
    ...
    async def async_run(self):
        ...
        while running:
            if self.menu.active:
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        # Toggle voice chat with V key
                            ...
                            if self.voice_mode:
                                print("Voice chat enabled - Recording...")
                                asyncio.create_task(self.handle_voice_chat())
                            else:
                                print("Voice chat disabled")
                                self.realtime_conv.is_recording = False
                                if self.realtime_conv.ws:
                                    await self.realtime_conv.ws.close()
                                    self.realtime_conv.ws = None
                ...
                # Create a surface for 2D rendering (messages)
                surface_2d = pygame.Surface((WINDOW_WIDTH, WINDOW_HEIGHT), pygame.SRCALPHA)
                
                # Render voice chat message if active
                self.realtime_conv.render_message(surface_2d)
                
                # Convert surface to OpenGL texture and render
                if self.realtime_conv.current_message:
                    texture_data = pygame.image.tostring(surface_2d, "RGBA", True)
        ...
        if self.realtime_conv.ws:
            await self.realtime_conv.ws.close()
        self.realtime_conv.cleanup()
        pygame.quit()

    def run(self):
        """Start the game loop."""
        self.event_loop.run_until_complete(self.async_run())
